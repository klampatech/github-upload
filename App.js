...
function DetailsScreen({ route, navigation }) {
  ...
  return (
    <View style={{ flex: 1, justifyContent: 'space-between', alignItems: 'center', paddingBottom: 40, paddingTop: 40 }}>
      <React.Fragment>
        <View>
          <Text style={{fontSize: 30, paddingBottom: 10}}>{otherParam.name}</Text>
          <Text style={{fontSize: 15}}>{otherParam.company.name}</Text>
        </View>
        <View>
          <Text style={{fontSize: 25, fontWeight: 'bold', paddingBottom: 10}}>Contact Information</Text>
          <Text style={{ paddingBottom: 20 }}>{otherParam.email}</Text>
          <Text>{otherParam.address.street}</Text>
          <Text>{otherParam.address.suite}</Text>
          <Text>{otherParam.address.city}</Text>
          <Text>{otherParam.address.zipcode}</Text>
          <Text>{otherParam.phone}</Text>
        </View>
        <View>
          <Text style={{fontSize: 25, fontWeight: 'bold', paddingBottom: 10}}>Other Information</Text>
          <Text>{otherParam.website}</Text>
          <Text>{otherParam.company.catchPhrase}</Text>
          <Text>{otherParam.company.bs}</Text>
        </View>
      </React.Fragment>
    </View>
  );
